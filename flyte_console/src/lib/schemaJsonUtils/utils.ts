...
/** Task input JSON (JSON editor / API); recursive JSON shape. */
export type JsonFormData =
  ...
  | undefined
  | JsonFormData[]
  | { [key: string]: JsonFormData }

// ---------------------------------------------------------------------------
// Backend value normalization (NONETYPE, enums, defaults)
// ---------------------------------------------------------------------------

/** Backend can send Python None as { type: "null" }; normalize to actual null for UI/JSON. */
 ...
 * so the JSON editor shows null instead of [object Object].
 */
...
export function convertRjsfInternalFormats(
  ...
  if (
    ...
    return enumDefault as JsonFormData
  }
  ...
  if (!isObject(formValues) || formValues === null) {
    return formValues
  }
  const record = formValues as Record<string, JsonFormData>
  const valuesToUpdate = Object.entries(s.properties).reduce(
    (acc: Record<string, JsonFormData>, [key, schemaEntry]) => {
      if (schemaEntry === true || schemaEntry === false) return acc
      const entrySchema = schemaEntry as JSONSchema7
      if (!(key in record)) return acc
      let raw = record[key]
      const resolved = getSchema(entrySchema, root)
      if (resolved.type === 'array') {
        raw = ensureArrayIfNumericKeys(raw) as JsonFormData
      }
      acc[key] = convertRjsfInternalFormats(entrySchema, raw, root)
      return acc
    },
    {} as Record<string, JsonFormData>,
  )
  return { ...record, ...valuesToUpdate }
}

/**
 * Normalize form data: durations, enum nulls → first enum value, and $ref resolution.
 * Safe to call on every JSON form change and again at submit.
 */
...
export const skipUndefinedFormValues = (
  ...
  return Object.entries(formValues).reduce<Record<string, JsonFormData>>(
    (newValues, [key, val]) => {
      if (val !== undefined) {
        newValues[key] = skipUndefinedFormValues(val as JsonFormData)
      }
      return newValues
    },
    {},
