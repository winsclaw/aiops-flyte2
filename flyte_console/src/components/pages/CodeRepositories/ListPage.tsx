...
function idKey(repository: CodeRepository) {
  return repository.id?.id ?? "";
}

export function CodeRepositoryListPage() {
  const params = useParams<ProjectDomainParams>();
  const org = useOrg();
  const client = useConnectRpcClient(CodeRepositoryService);
  const [items, setItems] = useState<CodeRepository[]>([]);
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      setItems(response.codeRepositories ?? []);
    } catch (error) {
  ...
  const filteredItems = useMemo(
    () =>
      items.filter((item) =>
        searchTerm
          ? `${item.repoUrl} ${item.branch} ${item.mountPath}`
              .toLowerCase()
    ...
    try {
      await Promise.all(
        selectedItems.map((item) =>
          client.deleteCodeRepository(
            create(DeleteCodeRepositoryRequestSchema, { id: item.id }),
          ),
        ),
      );
      setSelected(new Set());
      ...
      console.error("Error deleting code repositories", error);
      setMessage("删除代码库失败");
    } finally {
            ...
            <table className="min-w-full divide-y divide-zinc-200 text-left dark:divide-zinc-800">
              <thead className="sticky top-0 bg-white dark:bg-zinc-950">
                <tr className="text-sm font-semibold text-zinc-600 dark:text-zinc-300">
                  ...
                  <th className="px-4 py-4">地址</th>
                  <th className="px-4 py-4">分支</th>
                  <th className="px-4 py-4">挂载路径</th>
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {filteredItems.map((item) => (
                  ...
                  >
                    ...
                    <td className="max-w-xl px-4 py-4 font-medium text-blue-600">
                      <span className="inline-flex items-center gap-2">
                        <CodeBracketIcon className="size-4 text-zinc-500" />
                        <span className="break-all">{item.repoUrl}</span>
                      </span>
                    </td>
                    <td className="px-4 py-4">{item.branch}</td>
                    <td className="px-4 py-4">{item.mountPath}</td>
