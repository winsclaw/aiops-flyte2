...
export const colors = {
  gray: 'dark:bg-(--system-gray-5) dark:text-black bg-(--system-gray-5)',
  red: 'bg-red-500/15 text-red-700 group-data-hover:bg-red-500/25 dark:bg-red-500/10 dark:text-red-400 dark:group-data-hover:bg-red-500/20',
  orange:
    'bg-orange-500/15 text-orange-500 group-data-hover:bg-orange-500/25 dark:bg-orange-500/10 dark:text-orange-300 dark:group-data-hover:bg-orange-500/20',
  amber:
  ...
  zinc: 'bg-zinc-600/10 text-zinc-700 group-data-hover:bg-zinc-600/20 dark:bg-white/5 dark:text-zinc-500 dark:group-data-hover:bg-white/10',
}

export type BadgeProps = HTMLAttributes<'span'> & {
...
export function Badge({
  ...
  return (
    <span
      ...
      className={clsx(
        className,
        'font-small inline-flex items-center gap-x-1.5 rounded-md px-1.5 py-0.5 text-xs/5 sm:text-xs/5 forced-colors:outline',
        colors[color],
      )}
    />
  )
...
export const BadgeButton = function BadgeButton({
  ...
  const classes = clsx(
    className,
    'group relative inline-flex rounded-md focus:outline-hidden data-focus:outline-2 data-focus:outline-offset-2 data-focus:outline-blue-500 cursor-pointer',
  )
