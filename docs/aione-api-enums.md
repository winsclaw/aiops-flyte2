...
`/v2/api/aione/{id}/status` 返回的 `data.phase` 字段是 Flyte 2 `flyteidl2.common.ActionPhase` 的数字枚举值。
...
```text
UNSPECIFIED, QUEUED, WAITING_FOR_RESOURCES, INITIALIZING, RUNNING
```

## 状态查询接口字段

`GET /v2/api/aione/{id}/status` 返回标准响应结构：
